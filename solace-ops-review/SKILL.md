...
for dir in topic-design sam-design broker-select protocol-select mesh-design ha-dr integration migration; do
...
- What metrics are critical? (Message rate, spool usage, connection count, queue depth,
  replication lag, DMR link health)
...
- **Critical (P1)** — Missing monitoring, unhandled failure modes, no runbook
  for critical paths. Production risk.
- **Important (P2)** — Capacity limits that will be hit within the stated growth
  trajectory. Scaling risk.
- **Advisory (P3)** — Missing automation, manual processes that should be
...
```bash
ACTIVE=$(cat projects/.active)
mkdir -p "projects/$ACTIVE/artifacts/reviews"
cat > "projects/$ACTIVE/artifacts/reviews/ops-review.md" << 'EOF'
