...
Present the topology options via AskUserQuestion:

**A) Single broker (recommended for simple deployments):**
One broker handles all traffic. Simplest to operate. Right when all producers and
consumers are in the same site and throughput fits within one broker's capacity.

**B) DMR cluster (horizontal scaling within a site):**
Multiple brokers connected via internal links forming a full mesh. Each node connects
to every other node. Use when a single broker can't handle the connection count or
throughput, but all traffic is within one site.

**C) Multi-site federation via external links:**
Brokers across sites connected via external links. Full mesh is not required — selective
links allow controlled subscription propagation. Use for multi-region, multi-cloud, or
hybrid deployments.

**D) Hybrid (cluster + federation):**
DMR clusters within sites, external links between sites. The most complex topology
but necessary for large-scale global deployments.
...
Recommend next steps — `/solace-ha-dr` for HA and DR design, or `/solace-protocol-select`.
