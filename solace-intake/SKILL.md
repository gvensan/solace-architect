...
If the HTML builder is not found, suggest running `./setup` from the Solace Architect
repo to install all assets. The HTML form needs `PyYAML` for routing-rule embedding;
if missing, install with `pip install pyyaml` and re-run.
