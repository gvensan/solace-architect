...
cat "projects/$ACTIVE/artifacts/06-mesh-design/dmr-topology.md" 2>/dev/null || echo "NO_MESH"
...
EOF
cat > "projects/$ACTIVE/artifacts/07-ha-dr/ha-dr-topology.mermaid" << 'EOF'
...
- Primary: `/solace-protocol-select` — Protocol Selection (if not yet complete)
- Alternate: `/solace-integration` — Integration Design (if protocols are already decided)
